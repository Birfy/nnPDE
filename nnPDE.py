...
class NNPDE:
    ...
    def __init__(self, batch_size, N, d):
        ...
        self.u = self.bsubnetwork(
            self.x, True) + self.B(self.x) * self.subnetwork(self.x, False)
        ...
        with tf.variable_scope("inner"):
            for i in range(self.N):
                x = tf.layers.dense(
                    ...
                    reuse=reuse)
            x = tf.layers.dense(
                x, 1, activation=None, name='output', reuse=reuse)
        ...
        with tf.variable_scope("boundary"):
            for i in range(self.N):
                x = tf.layers.dense(
                    ...
                    64,
                    activation=tf.nn.tanh,
                    name="bdense{}".format(i),
                    reuse=reuse)
            x = tf.layers.dense(
                x, 1, activation=None, name="boutput", reuse=reuse)
            x = tf.squeeze(x, axis=1)
        return x
    ...
    def calculate(self, sess, X):
        ...
        '''
        value = sess.run(self.u_b, feed_dict={self.x_b: X})
        return value


class Problem2(NNPDE):
    def A(self, x):
        # return tf.pow(tf.constant(30.0, dtype=tf.float64), x[:, 0])
    ...
    def loss_function(self):
        deltax = compute_delta(self.u, self.x, 2)
        res = tf.reduce_mean((deltax)**2)
        return res
    ...
    def train(self, sess, i):
        ...
        bX = np.random.rand(4 * self.batch_size, self.d)
        for j in range(2):
